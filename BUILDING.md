...
- Add a `build` script to `package.json` and commit it so `npm run build` works out-of-the-box.
- Add a small `npm run serve` script for the dev server.

(Next step: I'll mark the documentation task as completed and verify the file was created.)
